...
import { ChevronRight, Info, RotateCcw, Sparkles } from "lucide-react";
...
export default function MorePage() {
  ...
  return (
    <div className="page-pad">
      ...
      <section className="mt-7 rounded-2xl border border-line bg-paper p-5">
      ...
      <div className="mt-5 overflow-hidden rounded-xl border border-line bg-white">
        ...
        <button type="button" onClick={resetDemo} className="flex w-full items-center justify-between p-4 text-left"><span className="flex items-center gap-3 text-sm font-medium"><RotateCcw size={18} className="text-coral" /> デモデータを初期化</span><span className="text-[11px] text-ink/40">一時状態のみ</span></button>
      </div>
    </div>
  );
}

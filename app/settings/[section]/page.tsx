...
import { APP_THEMES, usePreferences } from "@/lib/preferences-context";
...
export default function SettingDetailPage() {
  ...
  return (
    <div className="page-pad">
      ...
      {section === "style" && (
        <section className="settings-card mt-7" aria-labelledby="style-heading">
          <h2 id="style-heading" className="text-base font-semibold text-ink">テーマカラー</h2>
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2.5 rounded-xl border px-3 py-3 text-left text-xs font-medium transition ${
                    selected ? "border-coral bg-paper text-ink shadow-sm" : "border-line bg-white text-ink/65 hover:border-coral/55"
                  }`}
                >
                  <span
                    className={`theme-swatch h-5 w-5 shrink-0 rounded-full border-2 border-white ${selected ? "ring-2 ring-coral/25" : ""}`}

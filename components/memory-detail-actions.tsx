        ...
        <form onSubmit={handleUpdate} className="rounded-2xl border border-line bg-paper/55 p-4" aria-busy={busy === "update"}>
          ...
          <fieldset disabled={Boolean(busy)} className="mt-4 space-y-3 disabled:opacity-60">
            ...
            <label className="block text-xs font-medium text-ink">
              ...
              <textarea value={letter} onChange={(event) => setLetter(event.target.value.slice(0, MAX_MEMORY_LETTER_LENGTH))} maxLength={MAX_MEMORY_LETTER_LENGTH} rows={8} placeholder="あの日のことや、未来の自分へ残したい言葉を自由に書けます。" className="mt-1.5 w-full resize-y rounded-xl border border-line bg-ivory px-3 py-2.5 text-sm leading-7 outline-none placeholder:text-ink/30 focus:border-coral focus:ring-2 focus:ring-coral/10" />

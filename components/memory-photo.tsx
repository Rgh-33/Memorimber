...
export function MemoryPhoto({ src, alt, className = "", detailed = false }: {
  src: string; alt: string; className?: string; detailed?: boolean;
}) {
  ...
  return <img src={src} alt={alt} onError={() => setFailedSource(src)} className={className} />;
}

...
export function selectMemoryReminder(userId: string, date: string, memories: ReminderMemory[], items: Pick<MemoryTreeItem, "stage">[], preferences: NotificationPreferences = defaultNotificationPreferences()): Reminder | null {
  ...
  if (preferences.harvest_enabled && items.some((item) => item.stage === "quiz-ready")) groups.push([{
    type: "harvest", candidateId: null, title: "思い出の木",
    body: "収穫できる実が育っています。あの日のことを思い出してみませんか？", href: "/",
  }]);
  const anniversaries: Reminder[] = [];
  for (let yearsAgo = 1; preferences.anniversary_enabled && yearsAgo <= 3; yearsAgo++) {

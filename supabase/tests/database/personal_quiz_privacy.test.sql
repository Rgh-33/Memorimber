...
select ok(not has_function_privilege('authenticated','public.server_personal_quiz_media(uuid,uuid,text)','EXECUTE'),'clients cannot request internal media paths');
select ok(not has_function_privilege('anon','public.answer_personal_quiz(uuid,text)','EXECUTE'),'anonymous grading is forbidden');
select is(public.server_personal_quiz_media('93000000-0000-4000-8000-000000000002',(select (dto->>'id')::uuid from quiz_privacy_cases where kind='photo-to-caption'),null),null::text,'another user cannot resolve a question image');
select is(public.server_personal_quiz_media('93000000-0000-4000-8000-000000000001',(select (dto->>'id')::uuid from quiz_privacy_cases where kind='caption-to-photo'),null),null::text,'caption question cannot fetch hidden target image');
select is(public.server_personal_quiz_media('93000000-0000-4000-8000-000000000001',(select (dto->>'id')::uuid from quiz_privacy_cases where kind='photo-to-caption'),(select dto->'choices'->0->>'id' from quiz_privacy_cases where kind='photo-to-caption')),null::text,'text choices cannot be resolved to images for target comparison');
select ok(public.server_personal_quiz_media('93000000-0000-4000-8000-000000000001',(select (dto->>'id')::uuid from quiz_privacy_cases where kind='caption-to-photo'),(select dto->'choices'->0->>'id' from quiz_privacy_cases where kind='caption-to-photo')) is not null,'authorized photo choice resolves internally');
